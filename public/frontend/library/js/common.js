...
let rattingElement = $("input.rating-disabled");
if ( rattingElement.length > 0) {
    rattingElement.rating({
...
$(document).ready(function () {
    ...
    let cSlider__single = $('.cSlider--single');
    if( cSlider__single.length > 0 ){
        cSlider__single.slick({
            slide: '.cSlider__item',
            slidesToShow: 1,
            slidesToScroll: 1,
            arrows: true,
    ...
    let cSlider__nav = $('.cSlider--nav');
    if( cSlider__nav.length > 0 ) {
        cSlider__nav.on('init', function (event, slick) {
    ...
    $(document).on('click', '.js_item_variable:not(.disabled)', function (e) {
        ...
        var countKey = $(this).attr('data-countKey');
