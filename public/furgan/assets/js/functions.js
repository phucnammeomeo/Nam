(function ($) {
    "use strict";
    function furgan_video() {
        $('.buttonvideo').simpleLightboxVideo();
    }
    function furgan_google_maps() {
        if ($('.furgan-google-maps').length <= 0) {
            return;
        }
        $('.furgan-google-maps').each(function () {
            var $this = $(this),
                $id = $this.attr('id'),
    ...
    function furgan_hover_product_item($elem) {
        $elem.each(function () {
    ...
    $(window).on("load", function () {
        furgan_video();
        furgan_google_maps();
        if ($('.furgan-threed-view').length > 0) {
            furgan_threesixty();
        }
        ...
        if ($('.owl-slick .product-item').length) {
            furgan_hover_product_item($('.furgan-products .owl-slick .row-item,' +
