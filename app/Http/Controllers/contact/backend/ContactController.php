...
class ContactController extends Controller
{
    ...
    {
        $data = Contact::where('type', 'book')->orderBy('id', 'DESC');
        if (is($request->keyword)) {
            $data =  $data->where(function ($query) use ($request) {
                $query->where('email', 'like', '%' . $request->keyword . '%')
                    ->orWhere('fullname', 'like', '%' . $request->keyword . '%')
    ...
    {
        $data = Contact::where('type', 'machining')->orderBy('id', 'DESC');
        if (is($request->keyword)) {
            $data =  $data->where(function ($query) use ($request) {
                $query->where('email', 'like', '%' . $request->keyword . '%')
                    ->orWhere('fullname', 'like', '%' . $request->keyword . '%')
        ...
        $module =  'contacts';
        return view('contact.backend.machining', compact('module', 'data'));
    }
    public function machiningEdit($id)
    {
        $detail = Contact::find($id);
        if (!isset($detail)) {
            return redirect()->route('machining.index')->with('error', "Không tồn tại");
        }
        $module =  'contacts';
        return view('contact.backend.machiningEdit', compact('module', 'detail'));
    }
