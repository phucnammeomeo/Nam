...
class ProductController extends Controller
{
    ...
    public function __construct()
    {
        ...
        View::share(['taxesType' => $taxesType]);
    }
    public function index(Request $request)
    {
        ...
        $configIs = \App\Models\Configis::select('title', 'type', 'id')->where(['module' => $this->table, 'active' => 1])->get();
    ...
    public function create()
    {
        ...
        return view('product.backend.product.create', compact('field', 'module', 'address', 'listTaxes', 'htmlCatalogue', 'htmlAttribute', 'tags', 'getTags', 'htmlBrands', 'attribute_json', 'dropdown','configIs'));
    }
    ...
    public function edit($id)
    {
        ...
        $address = \App\Models\Address::select('id', 'title', 'cityid', 'districtid', 'wardid')
            ...
            ->get();
        $listTaxes = \App\Models\Tax::select('id', 'title', 'value')->where('id', '!=', 1)->get();
        return view('product.backend.product.edit', compact('field', 'module', 'address', 'listTaxes', 'getCatalogue', 'detail', 'htmlCatalogue', 'htmlAttribute', 'tags', 'getTags', 'htmlBrands',  'attribute_json', 'dropdown', 'polylang'));
    }
    public function copy($id)
    {
        ...
        return view('product.backend.product.copy', compact('field', 'module', 'address', 'listTaxes', 'getCatalogue', 'detail', 'htmlCatalogue', 'htmlAttribute', 'tags', 'getTags', 'htmlBrands', 'attribute_json', 'dropdown'));
    }
    ...
    public function submit($request = [], $action = '', $id = 0, $image_url = '')
    {
        ...
        if($action == 'update'){
            if(in_array("72",$tmp_catalogue)){
                if(!empty($request->price_sale)){
                    $tmp_catalogue = array_unique($tmp_catalogue);
                }
                else{
                    if($key = array_search("72", $tmp_catalogue)){
                        unset($tmp_catalogue[$key]);
                    }
                }
            }
            else{
        ...
        if (!empty($id)) {
            ...
            if (empty($checkbox)) {
                $this->Helper->insert_product_stocks($request['inventoryQuantity'], $request['stockAddress'], $id);
            }
