...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryProduct::select('id', 'slug', 'title', 'ishome', 'highlight', 'isaside', 'isfooter', 'order', 'publish', 'lft', 'rgt', 'level', 'created_at', 'userid_created', 'parentid')->with('user')->with('countProduct')->where('alanguage', config('app.locale'))->orderBy('lft', 'ASC');
    ...
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        $_data = [
            ...
            'title_category_description' => $request['title_category_description'],
