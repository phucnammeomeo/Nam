...
class ShipController extends Controller
{
    ...
    public function update_province(Request $request)
    {
        ...
        \App\Models\VNCity::find($id)->update($_data);
        return redirect()->route('ships.index_province')->with('success', "Cập nhật thông tin vận chuyển thành công.");
    }

    public function edit_province(Request $request)
    {
        $id = $request->id;
        $module = $this->table;
        $detail  = \App\Models\VNCity::orderBy('id', 'asc')->find($id);
        if (!isset($detail)) {
            return redirect()->route('ships.index')->with('error', "Hãng vận chuyển không tồn tại");
        }
        return view('ship.backend.updateProvince', compact('detail', 'module'));
    }
