...
class Helper
{
    ...
    //Them moi phien ban san pham: product_versions
    function insert_product_stocks($inventoryQuantity = 0, $stockAddress = [], $id)
    {
    ...
    function update_history_keyword( $keyword = '' )
    {
        $searchKeyword = Session::get('keyword');
        if( isset($searchKeyword) && is_array($searchKeyword) && count($searchKeyword) ){
            if( in_array(request()->keyword, $searchKeyword) ){
                $searchKeyword = array_diff($searchKeyword, [request()->keyword]);
            }
            array_unshift($searchKeyword, request()->keyword);
            Session::put('keyword', $searchKeyword);
            Session::save();
        } else {
            $key[] = request()->keyword;
        ...
        $listKeyWord = Session::get('keyword');
        return $listKeyWord;
    }
