...
if (!function_exists('getProductHighLight')) {
    function getProductHighLight( $lang = 'vi' )
    {
        $data = \App\Models\Product::select('id', 'title', 'image', 'slug', 'price', 'price_sale', 'price_contact')->where(['highlight' => 1, 'alanguage' => $lang, 'publish' => 0])->orderBy('order', 'asc')->orderBy('id', 'desc')->limit(5)->get();
        return $data;
    }
}