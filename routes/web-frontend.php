...
use App\Http\Controllers\keyword\frontend\KeywordController;
...
Route::group(['middleware' => 'locale'], function () {
    ...
    Route::get('/products/ao-oversize-shopify-test.html', [HomeController::class, 'products'])->name('homepage.products');
    Route::get('/api/merchant/campaigns', [HomeController::class, 'merchant']);
    Route::get('/gioi-thieu', [PageController::class, 'aboutUs'])->name('pages.aboutus');
    ...
    Route::post('/machining-create', [ContactController::class, 'machining'])->name('contactFrontend.machining');
    Route::post('/request-a-quote-create', [ContactController::class, 'quote'])->name('contactFrontend.quote');
    Route::post('/subcribers', [ContactController::class, 'subcribers'])->name('contactFrontend.subcribers');
    Route::post('/book-a-table', [ContactController::class, 'bookTable'])->name('contactFrontend.bookTable');
    Route::get('/tim-kiem', [CategoryController::class, 'search'])->name('homepage.search');
    Route::post('/quick-view', [ProductController::class, 'quickView'])->name('products.quickView');
    Route::get('/search', [FrontendCategoryController::class, 'search'])->name('homepage.search2');
    Route::get('/search', [FrontendCategoryController::class, 'search'])->name('homepage.search2');
    Route::post('/load-more-product', [AjaxController::class, 'load_more_product'])->name('products.loadmore');
    Route::post('/get-keyword-suggest', [KeywordController::class, 'getKeyword'])->name('keywords.getKeyword');
    ...
    Route::group(['prefix' => 'gio-hang'], function () {
        ...
        Route::post('/get-ship-infor', [ShipController::class, 'getShipInfor'])->name('cart.getShipInfor');
