...
<div class="banner-wrapper has_background">
    <img style="width: 100%;" src="{{asset($fcSystem['banner_3'])}}" alt="Kết quả tìm kiếm" class="img-responsive attachment-1920x447 size-1920x447">
                ...
                <div class="blog-standard content-post">
                    ...
                    <article class="post-item post-standard post-195 post type-post status-publish format-standard has-post-thumbnail hentry category-light category-table category-life-style tag-light tag-life-style">
                        <div class="post-thumb">
                            <a href="{{route('routerURL',['slug' => $item->slug])}}"><img src="{{asset($item->image)}}" class="img-responsive attachment-1170x768 size-1170x768" alt="{{$item->title}}" width="1170" height="768" style="width: 100%;"></a>
                        </div>
                        <div class="post-info">
                            <h2 class="post-title"><a href="{{route('routerURL',['slug' => $item->slug])}}">{{$item->title}}</a></h2>
                            <div class="post-meta">
                                <div class="date">
                                    <a href="javascript:void(0)">{{ \Carbon\Carbon::parse($item['created_at'])->format('M')}} {{ \Carbon\Carbon::parse($item['created_at'])->format('d')}}, {{ \Carbon\Carbon::parse($item['created_at'])->format('Y')}} </a>
                                </div>
                                @if(!empty($item->name))
                                <div class="post-author">
                                    By:<a href="javascript:void(0)"> {{$item->name}} </a>
                                </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <a href="{{route('routerURL',['slug' => $item->slug])}}" class="readmore">Xem chi tiết</a>
                    </article>
                    @endforeach
