<div class="cart-sum relative">
    <a class="text-f14 text-white pr-[15px] flex items-center click-cart cursor-pointer">
        <span class="cart relative inline-block w-[45px] shopping-cart">
           <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-vubbuv" focusable="false"
            ...
            <span class="stt w-[20px] h-[20px] bg-yellow-400 rounded-full text-white text-f12 inline-block text-center leading-[20px] top-[-7px] left-[19px] absolute cart-quantity">{{$cart['quantity']}}</span>
        </span>
    ...
    <div class="nav-cart-sum z-10 mini-cart">
            ...
            @if( $cart['cart'] )
                ...
                <div class="item flex flex-wrap justify-between mb-[10px]">
                    <div class="img" style="width: 50px;">
                        <a href="{{ route( 'routerURL', ['slug' => $val['slug']] ) }}"><img src="{{ asset(getImageUrl('products', $val['image'], 'small')) }}" alt="{{ $val['title'] }}"></a>
                    </div>
                    <div class="nav-img pl-[10px]" style="width: calc(100% - 50px);">
                        <h3 class="text-f14" style="line-height: 20px;">{{ $val['title'] }}</h3>
                        @if( $val['unit'] != '' )
                        <p class="text-f13 leading-[16px]">ĐVT: {{ $val['unit'] }}</p>
                        @endif
                        <p class="text-f13 leading-[16px]">x {{ $val['quantity'] }} <span class="float-right text-Pimary_color">{{ str_replace( '.', ',', number_format($val['price'])) }} ₫</span></p>
                    </div>
                </div>
                @endforeach
            @else
        ...
        <div id="header-cart-action" style="{{ $cart['quantity'] == 0?'display:none':'' }}">
            <div class="sum-sum border-t border-b border-gray-100 p-[10px]">
                <p class="text-f14">
                    Có tổng <span class="cart-quantity">{{$cart['quantity']}}</span> sản phẩm
                    <span class=" float-right">
                    Tổng tiền:<span class="text-Pimary_color font-bold cart-total">{{ str_replace('.', ',', number_format($cart['total'])) }}</span>
                </span>
                </p>
            </div>
            <div class="xemchitiet text-right p-[10px]">
                <a href="{{ route('cart.index') }}" class="bg-Pimary_color py-[5px] px-[10px] inline-block text-f14 text-white">Xem chi tiết</a>
            </div>
        </div>
    </div>
</div>
