...
<div class="content">
    <div class="flex justify-between items-center mt-10">
        ...
        <div class="flex flex-col md:flex-row gap-2 justify-end md:items-center">
            ...
            <a href="{{route('payment_groups.index')}}" class="flex items-center space-x-1 btn btn-primary">
                <span>Loại phiếu</span>
            </a>
            @endcan
            @can('payment_vouchers_create')
            <a href="{{route('payment_vouchers.create')}}" class="btn btn-primary shadow-md mr-2">Thêm mới</a>
            @endcan
        </div>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div id="loadData" class=" col-span-12">
            @include('cashbook.payment.payment.data')
        </div>
    </div>
</div>
                ...
                swal({
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: 'POST',
                                url: "{{route('payment_vouchers.ajaxDelete')}}",
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                },
                                data: {
