...
if ($errors->any()) {
    ...
    /*Thực hiện get dữ liệu bảng product_stocks */
    $addressCheck = \App\Models\ProductStock::where(['product_id' => $detail->id, 'type' => 'variable'])->get();
    $collectionGroup = $addressCheck->groupBy('product_version_id')->all();
    $valueAddress = [];
    if (!empty($collectionGroup)) {
        foreach ($collectionGroup as $key => $item) {
            if (!empty($item)) {
                foreach ($item as $i) {
                    $valueAddress[$key][$i['address_id']] = $i['value'];
                }
            }
        }
    }
                                    ...
                                    <a href="javascript:void(0)" class="form-label mb-0 accordion w-full js_add_option ">
                                        ...
                                        <?php if (!empty($title_v)) { ?>
                                            <?php foreach ($title_v as $k => $v) {
                                                if ($v != '') { ?>
                                                    <input type="hidden" name="title_check[]" value="{{$v}}">
                                                    <span class="text-xs whitespace-nowrap text-success bg-success/20 pending  pending-success/20 rounded-full px-2 py-1 mr-1 ">{{$v}}</span>
                                                <?php } ?>
                                            <?php } ?>
                                        <?php } ?>
                                    </a>
                                ...
                                <div class="version_item_size hidden">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 mt-3">
                                        ...
                                        <div class="">
                                            ...
                                            <input type="text" value="<?php echo $price_import_version[$key] ?>" name="price_import_version[]" class="form-control int price" placeholder="">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <div class="mt-3">
                                            <div class="form-switch">
                                                <?php /*<select class="form-select selectStock" name="_stock_status[]">
                                                ...
                                                <input type="text" name="_stock_status[]" value="{{$_stock_status[$key]}}" class="hidden">
                                                <div class="form-check">
                                                    <input id="horizontal-form-{{$key}}" class="form-check-input selectStock" type="checkbox" value="1" @if($_stock_status[$key]==1) checked @endif>
                                                    <label class="form-check-label" for="horizontal-form-{{$key}}">Quản lý kho hàng</label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="showStock @if($_stock_status[$key]==0) hidden @endif">
                                                        ...
                                                        <tbody>
                                                            <?php if (!$address->isEmpty()) { ?>
                                                                <?php foreach ($address as $item) { ?>
                                                                    <tr>
                                                                        <td class="text-left p-2 "><span class="font-medium">
                                                                                <?php echo $item->title ?></span><br>
                                                                            <?php echo $item->ward_name->name ?>-<?php echo $item->district_name->name ?>-<?php echo $item->city_name->name ?>
                                                                        </td>
                                                                        <td class="text-right p-2">
                                                                            @if(!empty($_stock_address))
                                                                            <input class="form-control w-full int js_stock_address_value" autocomplete="off" name="_stock_address[<?php echo $key ?>][<?php echo $item->id ?>]" type="text" value="<?php echo $_stock_address[$key][$item->id] ?>">
                                                                            @else
                                                                            <input class="form-control w-full int js_stock_address_value" autocomplete="off" name="_stock_address[<?php echo $key ?>][<?php echo $item->id ?>]" type="text" value="<?php echo !empty($valueAddress) ? (!empty($valueAddress[$id_version[$key]]) ? $valueAddress[$id_version[$key]][$item->id] : 0)  : 0 ?>">
                                                                            @endif
                                                                        </td>
                                                                    </tr>
                                                                <?php } ?>
                                            ...
                                            <div class="mt-3">
                                                <input type="text" name="_outstock_status[]" value="{{$_outstock_status[$key]}}" class="hidden">
                                                <div class="form-check">
                                                    <input id="horizontal-status-{{$key}}" class="form-check-input selectStockStatus" type="checkbox" value="1" @if($_outstock_status[$key]==1) checked @endif>
                                                    <label class="form-check-label" for="horizontal-status-{{$key}}">Đồng ý cho đặt hàng khi đã hết hàng</label>
                                                </div>
                                                <?php /*<div class="form-switch">
                                                    <label class="form-label">Đặt hàng khi đã hết hàng</label>
                                                    <select class="form-select" name="_outstock_status[]">
                                                        <option value="0" @if(!empty($_outstock_status[$key]) && $_outstock_status[$key]==0) selected @endif>Không
                                                            cho đặt hàng khi hết hàng</option>
                                                        <option value="1" @if(!empty($_outstock_status[$key]) && $_outstock_status[$key]==1) selected @endif>Đồng
                                                            ý
