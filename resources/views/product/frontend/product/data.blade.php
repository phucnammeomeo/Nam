...
<div class="row flex flex-wrap justify-between -mx-3">
                                ...
                                @foreach($listAlbums as $key => $val)
                                    <div class="swiper-slide">
                                        <div class="slider__image">
                                            <img src="{{ asset($val) }}" alt="{{ $detail->title }}" />
                                        </div>
                                    </div>
                                @endforeach
                            @endif
    ...
    <div class="w-full md:w-2/5 px-3 lg:mt-0 md:mt-0 sm:mt-4 mt-4">
        ...
        <p class="price mt-[10px] border-b-[1px] pb-[10px]">
            @if($price['price_final'] == "Liên hệ")
                <a href="tel:{{$fcSystem['contact_hotline']}}"
                   title="Liên hệ hotline: {{$fcSystem['contact_hotline']}}"><span
                            class="text-f25 font-bold text-red-600"> {{ $price['price_final'] }} </span></a>
            @else
            ...
            @if (!empty($price['price_old']))
                ...
                ?>
                <del class="text-f16 text-gray-400 pl-[10px]">{{ $price['price_old'] }}
                </del>
                ...
                <div class="phone-phone">
                    <div class="flex flex-wrap justify-between mx-[-5px]">
                        <div class="w-full md:w-1/2 px-[5px]">
                            <a href="tel:{{$fcSystem['contact_hotline']}}" class="mb-[5px] md:mb-0 w-full inline-block py-[7px] text-white bg-Pimary_color text-center text-f16 font-bold rounded-[25px] border border-Pimary_color hover:bg-white hover:text-Pimary_color transition-all">
                                <i class="fa-solid fa-phone mr-2"></i>
                                {{$fcSystem['contact_hotline']}}
                            </a>
                        </div>
                        <div class="w-full md:w-1/2 px-[5px]">
                            <a href="tel:{{$fcSystem['contact_hotline2']}}" class="mb-[5px] md:mb-0 w-full inline-block py-[7px] text-white bg-Pimary_color text-center text-f16 font-bold rounded-[25px] border border-Pimary_color hover:bg-white hover:text-Pimary_color transition-all">
                                <i class="fa-solid fa-phone mr-2"></i>
                                {{$fcSystem['contact_hotline2']}}
                            </a>
                        </div>
                    </div>
                </div>
