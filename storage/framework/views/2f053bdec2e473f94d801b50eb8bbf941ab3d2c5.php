...
$shipping = !empty(old('shipping')) ? old('shipping') : (!empty($detail->shipping) ? $detail->shipping : '');
$endow = !empty(old('endow')) ? old('endow') : (!empty($detail->endow) ? $detail->endow : '');
...
$price_import =  !empty(old('price_import')) ? number_format(old('price_import'), '0', ',', '.') : (!empty($detail->price_import) ? number_format($detail->price_import, '0', ',', '.') : '');
$price =  !empty(old('price')) ? number_format(old('price'), '0', ',', '.') : (!empty($detail->price) ? number_format($detail->price, '0', ',', '.') : '');
$price_sale =  !empty(old('price_sale')) ? number_format(old('price_sale'), '0', ',', '.') : (!empty($detail->price_sale) ? number_format($detail->price_sale, '0', ',', '.') : '');
...
<div class=" box p-5">
    ...
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Mô tả ưu đãi vận chuyển cho sản phẩm</label>
        <div class="mt-2">
            <?php echo Form::textarea('shipping', $shipping, ['id' => 'ck_Description', 'class' => 'ck-editor-description', 'style' => 'height:60px;font-size:14px;line-height:18px;border:1px solid #ddd;padding:10px']); ?>
        </div>
    </div>

    <div class="mt-3">
        <label class="form-label text-base font-semibold">Mô tả ưu đãi sản phẩm</label>
        <div class="mt-2">
            <?php echo Form::textarea('endow', $endow, ['id' => 'cke_Description', 'class' => 'ck-editor-description', 'style' => 'height:60px;font-size:14px;line-height:18px;border:1px solid #ddd;padding:10px']); ?>
        </div>
    </div>
...
<div class="box p-5 mt-3 space-y-3 ">
    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="">
            ...
            <?php echo Form::text('price_sale', $price_sale, ['class' => 'form-control int ', 'autocomplete' => 'on']);; ?>
        </div>
        <div class="">
        ...
        <div class="col-span-3 hidden hide">
...
<div class=" box p-5 mt-3 space-y-3 <?php if (!in_array('ships', $dropdown)) { ?>hidden<?php } ?> hidden hide">
    <div>
        <label class="form-label text-base font-semibold">Vận chuyển</label>
    </div>
    <!-- START: Cân nặng, Kích cỡ(DxRxC) -->
    <div class="flex flex-col md:flex-row gap-2 hidden hide">
        <div class="w-1/2">
            <div>
...
<?php /**PATH /home/khangdien3/domains/khangdien.tamphat.edu.vn/public_html/resources/views/product/backend/product/common/_detail.blade.php ENDPATH**/ ?>