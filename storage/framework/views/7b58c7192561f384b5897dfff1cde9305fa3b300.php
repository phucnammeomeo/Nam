...
<div class="content">
    ...
    <div class="grid grid-cols-12 gap-6 mt-3">
        <div class=" col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2 justify-between">
            ...
            <a href="javascript:void(0)" class="btn btn-primary shadow-md mr-2" data-tw-toggle="modal" data-tw-target="#form_add_payment_groups">Thêm mới</a>
            <?php endif; ?>
        </div>
        <!-- BEGIN: Data List -->
                    ...
                    <tr class="odd " id="post-<?php echo $v->id; ?>" class="cursor-pointer">
                        ...
                        <td class="table-report__action w-56">
                            <div class="flex justify-center items-center">
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payment_vouchers_edit')): ?>
                                <a class="flex items-center mr-3 js_edit_payment_groups" href="javascript:void(0)" data-id="<?php echo e($v->id); ?>">
                                    <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>
                                    Edit
                                </a>
                                <?php endif; ?>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payment_vouchers_destroy')): ?>
...
<div id="form_add_payment_groups" class="modal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body grid grid-cols-12 gap-4 gap-y-3 FormAddSuppliersCategories">
                ...
                <div class="col-span-12 sm:col-span-6">
                    ...
                    <input id="modal-form-1" type="text" class="form-control" name="title" placeholder="Nhập tên loại phiếu chi">
                </div>
                <div class="col-span-12 sm:col-span-6">
                    <label for="modal-form-2" class="form-label">Mã</label>
                    <input id="modal-form-2" type="text" class="form-control" name="code" placeholder="Nhập mã loại phiếu chi">
                </div>
                <div class="col-span-12">
...
<div id="form_update_payment_groups" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <!-- BEGIN: Modal Header -->
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">
                    Cập nhập loại phiếu chi
                </h2>
            </div>
            <!-- END: Modal Header -->
            <!-- BEGIN: Modal Body -->
...
<script type="text/javascript">
    $(document).on('click', '.js_submitFormAdd', function(e) {
        ...
        $.ajax({
            url: "<?php echo route('payment_groups.store') ?>",
            type: 'POST',
            data: {
    ...
    $(document).on('click', '.js_edit_payment_groups', function(e) {
        e.preventDefault();
        $.ajax({
            url: "<?php echo route('payment_groups.edit') ?>",
            type: 'POST',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
            ...
            success: function(data) {
                if ($.isEmptyObject(data.error)) {
                    ...
                    const myModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#form_update_payment_groups"));
                    myModal.show();
                } else {
                    swal({
                        title: "Thông báo",
    ...
    $(document).on('click', '.js_submitFormUpdate', function(e) {
        ...
        var code = $('.FormUpdate input[name="code"]').val();
        var description = $('.FormUpdate textarea[name="description"]').val();
        var id = $(this).attr('data-id');
        $.ajax({
            url: "<?php echo route('payment_groups.update') ?>",
            type: 'POST',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                title: title,
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('dashboard.layout.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\evox.local\resources\views/cashbook/payment/group/index.blade.php ENDPATH**/ ?>