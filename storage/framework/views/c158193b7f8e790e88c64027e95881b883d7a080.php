...
<div class="content">
    <div class="flex justify-between items-center mt-10">
        ...
        <div class="flex flex-col md:flex-row gap-2 justify-end md:items-center">
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payment_vouchers_index')): ?>
            <a href="<?php echo e(route('payment_groups.index')); ?>" class="flex items-center space-x-1 btn btn-primary">
                <span>Loại phiếu</span>
            </a>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payment_vouchers_create')): ?>
            <a href="<?php echo e(route('payment_vouchers.create')); ?>" class="btn btn-primary shadow-md mr-2">Thêm mới</a>
            <?php endif; ?>
        </div>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
            ...
            <div class="dropdown w-full" id="myDropdown">
                ...
                <div class="dropdown-menu w-40">
                    <ul class="dropdown-content">
                        <li> <a href="javascript:void(0)" class="dropdown-item ajax-DeletePaymentVoucher">Hủy phiếu chi</a> </li>
                    </ul>
                </div>
            </div>
        </div>
        <form action="" class="col-span-10 grid grid-cols-12 gap-2" id="search" style="margin-bottom: 0px;">
            ...
            <div class="col-span-5 flex space-x-2">
                <input type="search" name="keyword" class="form-control " placeholder="Tìm kiếm theo mã phiếu chi, tham chiếu, mã chứng từ gốc" autocomplete="off" value="<?php echo request()->get('keyword') ?>">
        ...
        <div id="loadData" class=" col-span-12">
            <?php echo $__env->make('cashbook.payment.payment.data', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        </div>
    </div>
</div>
        ...
        <div class="mt-5 space-y-3">
            <div class="flex-auto space-y-1">
                ...
                <select multiple="multiple" class="form-control tom-select tom-select-custom tomselected" data-placeholder="Tìm kiếm..." style="height:42px" name="module" id="tomselect-1" tabindex="-1" hidden="hidden">
                    <?php if(!empty($table)): ?>
                    <?php $__currentLoopData = $table; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($key); ?>" <?php if(request()->get('module') == $key && $key != 0): ?> selected="selected" <?php endif; ?>><?php echo e($item); ?></option>
            ...
            <div class="flex-auto space-y-1">
                ...
                <select class="form-control tom-select tom-select-custom tomselected" data-placeholder="Tìm kiếm..." style="height:42px" name="address_id" multiple>
                    ...
                    <option value="<?php echo e($item->id); ?>" <?php if(request()->get('address_id') == $item->id): ?> selected <?php endif; ?>><?php echo e($item->title); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php endif; ?>
                </select>
            </div>
            ...
            <div class="flex-auto space-y-1">
                <label class="font-medium mb-2">Tìm kiếm theo mã phiếu chi, tham chiếu, mã chứng từ gốc</label>
...
<script>
    ...
    function getListObjectPaymentVouchers(page = 1) {
        $.ajax({
            url: "<?php echo route('payment_vouchers.ajaxSearch') ?>",
            type: 'POST',
            data: {
    ...
    $(document).on('click', '.ajax-DeletePaymentVoucher', function() {
        ...
        $('.checkbox-item:checked').each(function() {
            ...
            if (checked == 1) {
                swal({
                    title: "Thông báo",
                    text: "Không được hủy các phiếu chi Tự động, Thanh toán cho đơn nhập hàng và trạng thái “Đã hủy”",
                    type: "error"
                });
                return rv = false;
            }
        ...
        if (rv == true) {
            ...
            if (id_checked.length <= 0) {
                ...
                swal({
                        title: "Bạn chắc chắn muốn hủy phiếu chi này?",
                        text: 'Thao tác này sẽ hủy các phiếu chi bạn đã chọn. Thao tác này không thể khôi phục.',
                        type: "warning",
                        showCancelButton: true,
                        confirmButtonColor: "#DD6B55",
                        confirmButtonText: "Thực hiện!",
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: 'POST',
                                url: "<?php echo e(route('payment_vouchers.ajaxDelete')); ?>",
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                },
                                data: {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('dashboard.layout.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\evox.local\resources\views/cashbook/payment/payment/index.blade.php ENDPATH**/ ?>