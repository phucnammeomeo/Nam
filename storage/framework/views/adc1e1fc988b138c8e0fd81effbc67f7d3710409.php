...
<script type="text/javascript">
    ...
    function get_vesion(attributesVersion = []) {
        ...
        attrs.forEach(function(data, index) {
            ...
            item = item + '</div>';
            item = item + '</div>';
            item = item + '<div class="grid grid-cols-2 gap-3 mt-3">';
            ...
            item = item + '</div>';
            item = item + '</div>';
            item = item + '<div class="showStock box p-5 mt-3 hidden">';
            /*Số lượng trong kho hàng */
            item = item + '<div class="">';
            item = item + '<label class="form-label">Số lượng trong kho</label>';
            item = item + '<input type="number" name="_stock[]" min="0" class="form-control <?php if (in_array('addresses', $dropdown)) { ?>disabled<?php } ?>" <?php if (in_array('addresses', $dropdown)) { ?>style="opacity: 1;"<?php } ?> placeholder="" value="0">';
            item = item + '</div>';
            /*END:Số lượng trong kho hàng */
            /*Chi nhánh */
            item = item + '<div class="mt-3">';
            item = item + '<div class="scrollbar <?php if (!in_array('addresses', $dropdown)) { ?>hidden<?php } ?>" style="max-height: 700px;">';
            item = item + '<table class="w-full">';
            item = item + '<thead>';
            item = item + '<tr>';
            item = item + '<th class="text-left p-2">Chi nhánh</th>';
            item = item + '<th class="text-right p-2">Tồn kho</th>';
            item = item + '</tr>';
            item = item + '</thead>';
            item = item + '<tbody>';
            <?php if (!$address->isEmpty()) { ?>
                <?php foreach ($address as $item) { ?>
                    item = item + '<tr>';
                    item = item + '<td class="text-left p-2 ">';
                    item = item + '<span class="font-medium"><?php echo $item->title ?></span><br><?php echo $item->ward_name->name ?>-<?php echo $item->district_name->name ?>-<?php echo $item->city_name->name ?>';
                    item = item + '</td>';
                    item = item + '<td class="text-right p-2">';
                    item = item + '<input class="form-control w-full int js_stock_address_value" autocomplete="off" name="_stock_address[' + stt + '][<?php echo $item->id ?>]" type="text" value="0">';
                    item = item + '</td>';
                    item = item + '</tr>';
                <?php } ?>
            <?php } ?>
...
<?php $__env->stopPush(); ?><?php /**PATH /home/khangdien2/domains/khangdien.tamphat.edu.vn/public_html/resources/views/product/backend/product/common/script.blade.php ENDPATH**/ ?>